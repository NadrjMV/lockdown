...
def process_event(camera_id, event_date, event_id, processed_events):
    ...
    # Segurança extra: Ignora se não for a data de hoje
    today_zm = time.strftime("%Y-%m-%d")
    if event_date != today_zm:
        return
    ...
    # Cria a pasta garantindo permissões 775
    event_folder = ensure_event_folder(camera_id, event_id)
    
    # Se a pasta não existir ou sumiu (cleanup), aborta
    if not os.path.exists(event_folder):
        return

    for frame in sampled:
        # Verifica se o diretório ainda existe antes de chamar o DeepStack
        if not os.path.exists(event_folder):
            break
    ...
    daily = os.path.join(OUTPUT_DIR, date_str)
    ...
    if text == last_log_content:
        return

    group_str = "-".join(map(str, group_ids))
    filename = f"detections_log__ID_{camera_id}__{event_id}__{group_str}__{time_str}.json"
    path = os.path.join(camera_folder, filename)

    try:
        with open(path, "w", encoding="utf-8") as f:
            f.write(text)
        logging.info(f"✅ Evento {event_id} (cam {camera_id}) processado")
    except Exception:
